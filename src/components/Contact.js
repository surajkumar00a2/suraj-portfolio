    ...
    <section id="contact" className="py-20">
      <div className="max-w-content mx-auto px-6">
        ...
        >
          ...
          <a
            href={`mailto:${profile.email}`}
            className="inline-flex items-center gap-2 px-6 py-3 rounded-md border border-border text-zinc-300 font-medium text-sm hover:border-primary/30 transition-colors"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="mt-12 grid grid-cols-1 sm:grid-cols-3 gap-4">
          {links.map((link, index) => (
            <motion.a
              ...
              className="p-4 rounded-lg border border-border hover:border-primary/20 transition-colors flex items-center gap-3 group"
            >
              <div className="text-zinc-500 group-hover:text-primary transition-colors">
                {link.icon}
              </div>
              <div>
                <div className="text-[10px] text-zinc-600 uppercase tracking-wider">{link.label}</div>
                <div className="text-sm text-zinc-300 group-hover:text-primary transition-colors">{link.value}</div>
              </div>
            </motion.a>
          ))}
        </div>
